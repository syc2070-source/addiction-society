...
const AUDITED_MUTATION_ENDPOINTS = [
  ...
  'backend/src/upload/upload.controller.ts POST /api/upload/image',
  'backend/src/upload/upload.controller.ts POST /api/upload/pdf',
] as const;
...
describe('AUTH-001 controller containment', () => {
  ...
  it('requires JWT and the admin role on every non-auth mutation endpoint', () => {
    const missingProtection = mutationEndpoints
      .filter((endpoint) => !PUBLIC_AUTH_MUTATIONS.has(endpoint.key))
      .flatMap((endpoint) => {
