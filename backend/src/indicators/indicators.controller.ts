...
/**
 * 지표 API. 읽기는 공개(approved만), 쓰기(추출 트리거·승인)는 admin 권한 필요(AS-FIX-1).
 *  GET  /api/indicators            목록(domain 필터)
 *  GET  /api/observations          관측치 목록(indicator·geo 필터)
 *  GET  /api/indicators/:idOrCode  상세 + 관측치 시계열
 *  GET  /api/indicators/health              [admin] 환경 자가진단(python·pdfplumber·소스 URL)
 *  POST /api/indicators/extract-pdf         [admin] PDF 추출 트리거({source?}) → pending
 *  POST /api/indicators/observations/approve [admin] pending → approved(검수 승인)
 */
@Controller('api')
